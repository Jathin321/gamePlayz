...
const TournamentDetails = () => {
  ...
  const renderContent = () => {
    switch (activeTab) {
      ...
      case "PointsTable":
        return <p className="text-white">Points Table Content</p>;
      case "Teams":
        return <p className="text-white">Teams Content</p>;
      case "Announcements":
        return <p className="text-white">Announcements Content</p>;
      case "IDP":
        return <p className="text-white">IDP Content</p>;
      default:
        return <p className="text-white">Overview Content</p>;
    }
  };
  ...
  return (
    <div className="bg-[#171821]">
      {/* Banner */}
      <div className={`${classes.banner} lg:h-[250px] md:h-[200px] h-[150px] relative w-full mt-18`}>
        {/* Back Button */}
        <svg
      ...
      <div className="relative hidden px-20 md:block lg:block">
        <div className="flex flex-col md:flex-row items-center text-center md:text-left -translate-y-[45%]">
          ...
          <div className="w-full md:w-2/4 md:mb-0 text-white">
            ...
            <button className="bg-blue-600 text-white rounded-full px-4 py-2 mt-2">
              {tourney_data.status}
            </button>
          </div>
          ...
          <div className="w-full md:w-1/4 flex justify-center">
            <button className="w-full md:w-3/4 bg-blue-600 text-white rounded-full px-4 py-2">
              Join Tournament
            </button>
          </div>
        </div>
      ...
      <div className={`${classes.customActiveButton}custom-horizontal-nav bg-transparent mt-4 lg:hidden md:hidden`}>
            ...
            {navigationBar.map((tab) => (
              <button
                ...
                className={`btn custom-nav-button px-4 ${
                  activeTab === tab ? `${classes.underlineActive} text-white` : ""
                }`}
              >
                {tab}
              </button>
